        ...
        <div class="portlet light bordered">
        	<div class="portlet-title">
                ...
                <div class="grid actions">
                    @if($access['is_add'] ==1)
                    <a href="{{ url('admin/customer/update?return='.$return) }}" class="tips btn btn-xs green" title="Create {{ $pageTitle }}">
                    ...
                    @endif
                    <a href="javascript:;" class="tips btn btn-xs yellow-gold" onclick="_Modal('{{ url('admin/customer/search?return='.$return) }}','Advance Search'); return false;" title="Search {{ $pageTitle }}"><i class="fa  fa-search"></i> <span class="hidden-xs">Search</span></a>
                    @if($access['is_excel'] ==1)
                    <a href="{{ url('admin/customer/download?return='.$return) }}" class="tips btn btn-xs blue" title="Download {{ $pageTitle }}">
                        <i class="fa fa-cloud-download"></i> <span class="hidden-xs">Download</span></a>
                    @endif

                    <a href="{{ url('admin/customer') }}" class=" tips btn btn-xs btn-default" title="Clear Search"><i class="fa fa-spinner"></i> <span class="hidden-xs"> Clear</span></a>
                </div>
            </div>
            ...
            <div class="portlet-body">
                ...
                {!! Form::open(array('url'=>'admin/customer/delete/0?return='.$return, 'class'=>'form-horizontal' ,'id' =>'_Table' )) !!}
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            <li><a href="{{ url('admin/customer/show/'.$row->Id.'?return='.$return)}}" class="tips" title="View"><i class="fa  fa-search "></i> View </a></li>
                                            @endif @if($access['is_edit'] ==1)
                                            <li><a href="{{ url('admin/customer/update/'.$row->Id.'?return='.$return) }}" class="tips" title="Edit"><i class="fa fa-edit "></i> Edit </a></li>
                                            @endif
                                        </ul>
                                    </div>
                ...
                @else
                    <p class="text-center image_background no-result">There is no record!
                        <br />
                        <br />
                        <a href="{{ url('admin/customer/update?return='.$return) }}" class="btn btn-danger "><i class="icon-plus-circle2"></i> New {{ $pageTitle }} </a>
                    </p>
                @endif
            </div>
        </div>
