...
class TestController extends Controller {
	...
	function getUpdate(Request $request, $id = null)
	{
		...
		if($row)
		{
			...
			$this->data['row'] = $this->model->getColumnTable('User_profile'); 
			$this->data['pageAction'] =  'Add '.$this->info['title'];
		}
		$this->data['fields'] 		=  \SiteHelpers::fieldLang($this->info['config']['forms']);
		
		$this->data['id'] = $id;
		return view('test.form',$this->data);
	}	
	...
	public function getShow( Request $request, $id = null)
	{
		...
		if($row)
		{
			...
			return redirect('admin/test')->with('message','Record Not Found !')->with('status','error');					
		}
	}	
	...
	function postSave( Request $request)
	{
		...
		$rules = $this->validateForm($request, $request->input('id'));
		...
		if ($validator->passes()) {
			...
			{
				$return = 'admin/test/update/'.$id.'?return='.self::returnUrl();
			} else {
				$return = 'admin/test?return='.self::returnUrl();
			}
			...
			return redirect('admin/test/update/'.$request->input('id'))->with('message','The following errors occurred !')->with('status','error')
	...
	public function postDelete( Request $request)
	{
		...
		if(count($request->input('ids')) >=1)
		{
			...
			// redirect
			return redirect('admin/test?return='.self::returnUrl())
